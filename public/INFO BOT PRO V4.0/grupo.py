...
MAX_HISTORY = 50
SCAN_INTERVAL = 60          # 1 minuto — varredura automática
...
async def executar_varredura(user_client, notify_chat=None):
    """Varredura completa de todos os grupos públicos visíveis."""
...
# ══════════════════════════════════════════════
# 🧵  THREADS DE ATUALIZAÇÃO CONTÍNUA
# ══════════════════════════════════════════════

async def thread_atualizar_grupo(user_client, dialog):
    """Atualiza um grupo individual."""
    gid = str(dialog.id)
    nome_grupo = dialog.name
    db = carregar_dados()
    groups_db = carregar_grupos_db()
...
async def executar_threads_atualizacao(user_client):
    """Loop contínuo de threads de atualização."""
    global thread_scan_active
    while True:
        if not thread_scan_active:
            await asyncio.sleep(30)
            continue
        log("🧵 Threads de atualização iniciadas")
        total_changes = 0
        total_groups = 0
        try:
            async for dialog in user_client.iter_dialogs():
        ...
        log(f"🧵 Threads concluídas: {total_groups} grupos, {total_changes} alterações")
...
async def auto_scanner(user_client):
    """Varredura automática a cada SCAN_INTERVAL segundos."""
    while True:
